...
class AutoRunMazePane extends Pane {
	...
	public void paint() {
		...
		getChildren().clear();
		getChildren().addAll(startLabel,endLabel,startXText,endXText,startYText,endYText,
				runButton,xLabel,yLabel,fillButton);
	}
	...
	public void run(MazePane mazePane) {
		...
		fillButton.setOnAction(e->{
			startXText.setText("0");
			startYText.setText("1");
			int[][] tempData = mazePane.getMazaData();
		...
		runButton.setOnAction(e -> {
			...
			AutoRunMaze runMaze = new AutoRunMaze(mazePane.getMazaData(),new Integer(startX),
					new Integer(startY),new Integer(endX), new Integer(endY));
			mazePane.getChildren().clear();
			mazePane.paint(runMaze.getMazeData());
		});
